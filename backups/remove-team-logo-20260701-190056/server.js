...
const TEAM_LOGO_DIR = path.join(PUBLIC_DIR, 'images', 'team-logos');
...
function sanitizeFilename(value) {
  ...
  if (!/^[\w .()'-]+\.(png|jpg|jpeg|webp)$/i.test(name)) return '';
  return name;
}
...
function sanitizeTeam(team, fallback) {
  ...
  return {
    ...
    logo: sanitizeFilename(source.logo) || fallback.logo,
...
app.get('/api/team-logos', (req, res) => {
  res.json({ logos: listImageFiles(TEAM_LOGO_DIR) });
});
...
io.on('connection', (socket) => {
  ...
  controlEvent(socket, 'updateTeamLogo', ({ team, logo }) => {
    if (!isTeamKey(team)) return;
    gameState[team].logo = sanitizeFilename(logo) || '';
    emitState();
  });
