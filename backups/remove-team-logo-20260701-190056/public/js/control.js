...
async function boot() {
  const [heroesResult, draftResult, logosResult, presetsResult, stateResult] = await Promise.allSettled([
    fetchJson('/api/heroes'),
    fetchJson('/api/draft-sequence'),
    fetchJson('/api/team-logos'),
  ...
  teamLogos = valueOf(logosResult)?.logos || [];
  presetNames = valueOf(presetsResult)?.presets || [];
  latestState = valueOf(stateResult) || latestState;

  const failedLoad = [heroesResult, draftResult, logosResult, presetsResult, stateResult]
...
function renderLogoLists() {
  const datalist = document.getElementById('teamLogoList');
  if (!datalist) return;
  datalist.textContent = '';
  teamLogos.forEach((logo) => datalist.appendChild(new Option(logo, logo)));
}
...
function collectTeamState(color, fallback = {}) {
  return {
    ...
    logo: document.getElementById(`${color}TeamLogo`)?.value || '',
